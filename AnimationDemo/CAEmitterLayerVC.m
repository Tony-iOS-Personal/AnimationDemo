...
- (void)viewDidLoad {
    ...
    NSArray *buttonNames = @[@"大雪纷飞",@"心花怒放",@"百花争艳",@"璀璨烟花"];
    ...
    for (int i = 0; i < buttonNames.count; i++) {
        ...
        [aniButton addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)fireworks{
    // 发射源大小
    self.emitterLayer.emitterSize = CGSizeMake(self.view.width * 0.1, 0.f);
    // 发射位置 在底部向上发射
    self.emitterLayer.emitterPosition = CGPointMake(self.view.width * 0.5, self.view.height);
    // 以直线发射
